...
- Activation requires network and the configured product ID to match the activation response. The accepted product ID is persisted locally. On launch, cached keyed access is held until product identity is checked against current configuration and public validation completes. Successful validation refreshes the timestamp; explicit invalidation revokes access. A previously active license gets seven days of offline grace after its last successful validation; after that it returns to the gate until revalidated. A missing or different configured product ID never receives keyed access, even during offline grace. Legacy unlocked records without a key retain their migration entitlement.
