...
On the landing the pile does not only redden - it closes in. While a silence window runs (armed at t >= 1s), the gutter fossils of `#floodBase` advance on the paper in ranks and lanes, driven by one rAF loop on the shared silence clock; the 100ms ticker keeps owning fail/countdown, the rAF owns only continuous geometry. The math lives in `.pi/context/plans/siege-motion-model.md` (GPT-5.6 Sol advisory) and is contract here: per-fragment deadline progress `q^gamma` (front lead contacts exactly at 5.000s, mid lead at 6.000s, every seeded fragment holds contact by 7.000s - the wall itself is a second countdown); ease-in arrival with momentum, no deceleration, no spring; post-contact load aggregates into a per-side pressure integral `Q`; the shell (`.siege-shell`, the one transform owner wrapping paper + result/failure + footer) compresses `sx = 1 - 0.16 * P^1.10`, `sy = 1 - 0.035 * P^1.30` with a seeded asymmetric drift (`tx` clamp +/-6px, deadband) - a squeeze, never a zoom-out, and never before first contact. Reinforcements arrive from beyond the viewport on an irregular seeded cadence (540-780ms, side balance, lane spacing, cap 48) so the outside never empties; each has its own birth clock and a staging face behind the seed wall. Red and the veil stay danger-gated as the table below says; per-side flank shadows carry contact pressure only. Fragments keep their clean-column max-width caps; every travel is inward by construction.

The wipe is a collective kill: snapshot, 120ms pounce (`K(u) = u^2.60`) of every fragment (reserves included) to the 0.78-scaled edge with the shell slamming `scale(.78,.78)`, all fragments forced red (`body.siege-kill`), a 60ms hold, then a 320ms cubic settle back to rest while the lost draft's fossil flies out through the siege lines and joins them (the flight starts at the k = 120ms impact, then the existing flash + settle). One keystroke reverses the siege: colors and flanks clear in the first frame, geometry lerps snapshot -> rest over 180ms, `R(u) = 1 - (1-u)^3`, same curve for all ranks. `prefers-reduced-motion`: no siege motion ever (engine off); danger colors still change; wipe and fossil render statically. Mobile (<700px) keeps the table's behavior for now - no spatial siege there yet.
