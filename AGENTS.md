...
- The landing's danger is spatial: while a silence window runs, the pile closes in from both gutters (the siege, desktop only - see `design/DESIGN.md` section 7). Front rank contacts the paper's edge exactly at the 5s warn, the wall completes encirclement by countdown "1", the paper's `.siege-shell` compresses under the pressure integral only after first contact, reinforcements keep arriving from offscreen, and the wipe ends in a collective pounce + 0.78 slam before the fossil flies out through the siege lines. One keystroke reverses it in 180ms; `prefers-reduced-motion` keeps it fully static. The 100ms Datastar ticker owns discrete state; one rAF owns the continuous geometry; no signals are added.
